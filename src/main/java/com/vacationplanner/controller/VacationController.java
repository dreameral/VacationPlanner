...
public class VacationController extends RestAPIController {
  ...
  public List<GetVacationDTO> listAllLeaves(@RequestParam(value = "userId", required = false) Long userId,
  ...
  public ResponseEntity<?> createRequest(@RequestBody PostVacationDTO leaveDTO) throws ParseException {
  ...
  public ResponseEntity<?> updateRequest(@PathVariable("id") Long id, @RequestBody PostVacationDTO leaveDTO) throws ParseException {
    Vacation vacation = vacationService.getById(id);
  ...
  private List<GetVacationDTO> getByUserId(Long userId) {
    ...
    List<Vacation> leaves = vacationService.findByUser(user);

    for (Vacation vacation : leaves) {
      retVal.add(new GetVacationDTO(vacation));
    }
  ...
  private List<GetVacationDTO> getByStatus(String status) {
    ...
    List<Vacation> leaves = vacationService.findByStatus(vacationStatus);
  ...
  private List<GetVacationDTO> getByUserAndStatus(Long userId, String status) {
    ...
    List<Vacation> leaves = vacationService.findByUserAndStatus(user, vacationStatus);
